# -*- coding: utf-8 -*-
"""
Test on CIFAR-100 using ResNet-110 network model
"""
...
import pandas as pd
from dataset_100 import load_data
...
parser.add_argument('-n', '--stack_n', type=int, default=18, metavar='NUMBER',
...
if __name__ == '__main__':
    ...
    datagen.fit(train_x)
    history_1 = resnet_1.fit_generator(datagen.flow(train_x, train_y, batch_size=batch_size),
                                       steps_per_epoch=iterations, epochs=epochs, callbacks=cbks_sgd,
                                       validation_data=(test_x, test_y))
    #pd.DataFrame.from_dict(history_1.history).to_csv("resnet110_relu_log.csv", float_format="%.5f", index=False)

    history_2 = resnet_2.fit_generator(datagen.flow(train_x, train_y, batch_size=batch_size),
                                       steps_per_epoch=iterations, epochs=epochs, callbacks=cbks_sgd,
                                       validation_data=(test_x, test_y))
    #pd.DataFrame.from_dict(history_2.history).to_csv("resnet110_mish_log.csv", float_format="%.5f", index=False)

    history_3 = resnet_3.fit_generator(datagen.flow(train_x, train_y, batch_size=batch_size),
                                       steps_per_epoch=iterations, epochs=epochs, callbacks=cbks_sgd,
                                       validation_data=(test_x, test_y))
    #pd.DataFrame.from_dict(history_3.history).to_csv("resnet110_myfun_log.csv", float_format="%.5f", index=False)
    ...
    # plt.plot(history_2.history['loss'], label='Swish')
    ...
    # plt.plot(history_2.history['val_error'], label='Swish')

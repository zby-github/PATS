# -*- coding: utf-8 -*-
"""
Test on CIFAR-100 using ResNet-50 network model
"""
...
parser.add_argument('-n', '--stack_n', type=int, default=8, metavar='NUMBER',
...
if __name__ == '__main__':
    ...
    #pd.DataFrame.from_dict(history_1.history).to_csv("resnet50_relu_log.csv", float_format="%.5f", index=False)

    history_2 = resnet_2.fit_generator(datagen.flow(train_x, train_y, batch_size=batch_size),
                                       steps_per_epoch=iterations, epochs=epochs, callbacks=cbks_sgd,
                                       validation_data=(test_x, test_y))
    #pd.DataFrame.from_dict(history_2.history).to_csv("resnet50_mish_log.csv", float_format="%.5f", index=False)

    history_3 = resnet_3.fit_generator(datagen.flow(train_x, train_y, batch_size=batch_size),
                                       steps_per_epoch=iterations, epochs=epochs, callbacks=cbks_sgd,
                                       validation_data=(test_x, test_y))
    #pd.DataFrame.from_dict(history_3.history).to_csv("resnet50_myfun_log.csv", float_format="%.5f", index=False)
